...
import { demonstrationPortalProjects, mergePortalProject, portalProjectFromOpportunity, visiblePortalProjects, type PortalProject } from "@/lib/workflow-portal";
...
type Opportunity = { stable_id: string; name: string; stage: string; client?:string; solicitation?:string; canonical_url?:string; source?:string; source_updated_at?:string; is_demonstration?:boolean; qualification_score?: number; recommendation?: string; amount?: number; currency?: string; probability?: number; expected_close?: string; next_action?: string; next_action_date?: string; proposal_status?: string; recurring_potential?: string; required_approvals?: string[] };
    ...
    <div className="app-shell">
      ...
      <main>
        ...
        <div className="view-wrap">
          ...
          {view === "Action Portal" && <WorkflowPortal key={`${demoMode}-${workflowFocusId || "queue"}`} accessKey={accessKey} demoMode={demoMode} projects={workflowProjects} setProjects={setWorkflowProjects} focusProjectId={workflowFocusId} />}
...
function SalesOperations({ opportunities, source, onStartResponse }: { opportunities: Opportunity[]; source: string; onStartResponse:(opportunity:Opportunity)=>void }) {
  ...
  return <section>
    ...
    <div className="sales-table"><div className="sales-head"><span>Opportunity</span><span>Qualification</span><span>Commercial</span><span>Next action</span><span>Governance</span></div>{opportunities.map(o => { const weighted = Number(o.amount||0)*Number(o.probability||0)/100; const stale = Boolean(o.next_action_date && new Date(o.next_action_date)<new Date("2026-07-20")); return <button type="button" className={`sales-row sales-row-button ${selectedId === o.stable_id ? "selected" : ""}`} title={`Select ${o.name}`} onClick={()=>setSelectedId(o.stable_id)} key={o.stable_id}><div><p>{o.stable_id} · {o.stage}</p><strong>{o.name}</strong><small>{o.recommendation || "Not scored"}</small></div><div><b className="score-badge">{o.qualification_score ?? "—"}</b><small>/100</small></div><div><strong>{money(Number(o.amount||0))}</strong><p>{o.probability || 0}% · {money(weighted)} weighted</p><small>Close {o.expected_close || "Unknown"}</small></div><div><strong>{o.next_action || "Missing"}</strong><p className={stale ? "danger-text" : ""}>{o.next_action_date || "No date"}{stale ? " · STALE" : ""}</p></div><div><span className="status-chip">{o.proposal_status}</span><p>Recurring: {o.recurring_potential}</p><small>{o.required_approvals?.length ? `${o.required_approvals.length} approval(s)` : "No open approval"}</small></div></button>})}</div>
    {selected && <aside className="opportunity-brief-drawer sales-opportunity-drawer" role="dialog" aria-modal="true" aria-label={`Pipeline opportunity ${selected.name}`}><button type="button" className="drawer-close" onClick={()=>setSelectedId(null)} aria-label="Close pipeline opportunity">×</button><p className="eyebrow">Pipeline → Actions handoff</p><h3>{selected.name}</h3><p>{selected.stable_id} · {selected.stage} · {selected.client || "Client requires confirmation"}</p><div className="brief-scoreline"><div><span>Qualification</span><strong>{selected.qualification_score ?? "—"}</strong></div><div><span>Probability</span><strong>{selected.probability ?? 0}%</strong></div><div><span>Required approvals</span><strong>{selected.required_approvals?.length ?? 0}</strong></div></div><section><p className="eyebrow">Next action</p><p>{selected.next_action || "Complete the governed qualification record."}</p></section><section><p className="eyebrow">Handoff behavior</p><p>This creates or selects one Action queue record, preserves its source and confidence, and retains the qualification and bid/no-bid gates. It does not send, price, submit, or commit externally.</p></section><div className="brief-actions one-action"><button type="button" onClick={()=>onStartResponse(selected)}>Start governed response →</button></div></aside>}
  </section>;
}
