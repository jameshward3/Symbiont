    ...
    <div className="app-shell">
      ...
      <main>
        ...
        <div className="view-wrap">
          ...
          {view === "Sales Operations" && <SalesOperations opportunities={opportunities} source={sourceLabel} onNavigate={setView} />}
          {view === "Marketing" && <MarketingControl data={marketingData} />}
          {view === "Opportunity Scout" && <OpportunityScout data={scoutData} onNavigate={setView} />}
...
function SalesOperations({ opportunities, source, onNavigate }: { opportunities: Opportunity[]; source: string; onNavigate:(view:View)=>void }) { const recordKind=source.startsWith("Live")?"authorized operational records":source.includes("Verified")?"verified published opportunity records":"demonstration records"; return <section><SectionLead kicker="AGT-002 · Sales operations" title="Qualified work, clean handoffs." body={`${source}. Values shown below are ${recordKind}. Select any opportunity to open its source, matching, risks, and draft narrative.`} /><div className="sales-table"><div className="sales-head"><span>Opportunity</span><span>Qualification</span><span>Commercial</span><span>Next action</span><span>Governance</span></div>{opportunities.map(o => { const weighted = Number(o.amount||0)*Number(o.probability||0)/100; const stale = Boolean(o.next_action_date && new Date(o.next_action_date)<new Date("2026-07-20")); return <button type="button" className="sales-row sales-row-button" title={`Open evidence brief for ${o.name}`} onClick={()=>onNavigate("Opportunity Scout")} key={o.stable_id}><div><p>{o.stable_id} · {o.stage}</p><strong>{o.name}</strong><small>{o.recommendation || "Not scored"}</small></div><div><b className="score-badge">{o.qualification_score ?? "—"}</b><small>/100</small></div><div><strong>{money(Number(o.amount||0))}</strong><p>{o.probability || 0}% · {money(weighted)} weighted</p><small>Close {o.expected_close || "Unknown"}</small></div><div><strong>{o.next_action || "Missing"}</strong><p className={stale ? "danger-text" : ""}>{o.next_action_date || "No date"}{stale ? " · STALE" : ""}</p></div><div><span className="status-chip">{o.proposal_status}</span><p>Recurring: {o.recurring_potential}</p><small>{o.required_approvals?.length ? `${o.required_approvals.length} approval(s)` : "No open approval"}</small></div></button>})}</div></section>; }

function OpportunityScout({ data, onNavigate }: { data: ScoutData | null; onNavigate:(view:View)=>void }) {
