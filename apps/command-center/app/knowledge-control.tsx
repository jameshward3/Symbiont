...
export function KnowledgeControl({ data }: { data: KnowledgeData | null }) {
  ...
  return <section className="knowledge-view">
    <Hero />
    <div className={`knowledge-disclosure ${data.source === "d1" ? "live" : "demo"}`}><div><span className={`dot ${data.source === "d1" ? "green" : "amber"}`} /><b>{data.source === "d1" ? "AUTHORIZED LIBRARY" : "DEMONSTRATION INDEX"}</b></div><p>{data.activation}</p><span>UPDATED {dateTime(data.asOf)}</span></div>
    ...
    <div className="knowledge-grid index-grid">
      <div className="knowledge-panel"><PanelTitle kicker="Resource library" title="Find the right source" suffix={`${visible.length} result${visible.length === 1 ? "" : "s"}`} />
        ...
        {visible.map((item) => <ResourceRow item={item} key={item.id} />)}
    ...
    <div className="knowledge-grid context-grid">
      <div className="knowledge-panel intake-panel"><PanelTitle kicker="Legacy Exchange" title="Historical archive" suffix="Local index complete" /><div className="intake-state"><strong>{LEGACY_INTAKE.physicalFiles.toLocaleString("en-US")}</strong><span>restricted historical files indexed locally</span></div><p>The archive is useful for company history, past performance, technical precedent, and project reconstruction. Its source files remain outside this deployment until access and authority review are complete.</p><dl><div><dt>Unique files</dt><dd>{LEGACY_INTAKE.uniqueHashes.toLocaleString("en-US")}</dd></div><div><dt>Nested records</dt><dd>{LEGACY_INTAKE.nestedArchiveMembers.toLocaleString("en-US")}</dd></div><div><dt>Library connection</dt><dd>Summary only</dd></div><div><dt>Deletion</dt><dd>Human approval</dd></div><div><dt>Assessment</dt><dd>Draft v0.1</dd></div></dl></div>
