...
export async function GET(request: Request) {
  try {
    ...
    if (!records.length) return NextResponse.json({ ...demonstrationKnowledgeData, database: "connected_empty", activation: "D1 is connected but contains no approved knowledge records. The restricted legacy intake remains local and is represented here only by its sanitized overview." });
    ...
    return NextResponse.json({ ...demonstrationKnowledgeData, database: "unavailable", activation: "D1 is unavailable. Demonstration metadata and the sanitized legacy overview are shown; no indexing or write is being simulated." });
  }
}
