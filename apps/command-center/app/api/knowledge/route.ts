...
import { calculateKnowledgeQuality, demonstrationKnowledgeData, type BusinessArea, type AssetType, type Classification, type LifecycleStatus } from "@/lib/knowledge";
...
export async function GET(request: Request) {
  try {
    ...
    if (!isAuthorized(request)) return NextResponse.json({ ...demonstrationKnowledgeData, database: "authorization_required", activation: "Secure access is required for live knowledge. Demonstration metadata is shown; restricted records are never returned." });
    ...
    if (!records.length) return NextResponse.json({ ...demonstrationKnowledgeData, database: "connected_empty", activation: "D1 is connected, but no live knowledge has been indexed. Exchange intake remains pending upload." });
    ...
    return NextResponse.json({ ...demonstrationKnowledgeData, database: "unavailable", activation: "D1 is unavailable. Demonstration metadata is shown; no indexing or write is being simulated." });
  }
}
