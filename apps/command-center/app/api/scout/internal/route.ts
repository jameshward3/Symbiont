...
async function activateScout() {
  ...
  const agentRows = [
    ...
    { id:"AGT-009", name:"Opportunity Scout", mission:"Discover, verify, score, deduplicate, and route public buying signals.", authorityLevel:"L1", parentAgentId:"AGT-001", status:"Active — bounded pilot", version:"1.0.0", updatedAt:now },
  ];
  ...
  const queries = [
    { id:"QRY-AGT009-FEDERAL", name:"Federal building intelligence", queryText:"(BIM OR VDC OR reality capture OR digital twin OR asset information OR building data) AND (RFP OR RFQ OR RFI OR solicitation)", sourceCategory:"SAM.gov + official federal procurement", geography:"United States", cadence:"Weekdays 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Official pages/APIs only; verify terms and rate limits each run", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
    { id:"QRY-AGT009-PUBLIC", name:"State and local capital programs", queryText:"(renovation OR modernization OR expansion) AND (BIM OR scan OR digital twin OR facilities technology)", sourceCategory:"States, municipalities, schools, airports, transit, utilities", geography:"United States", cadence:"Weekdays 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Public issuer sources only; no account creation", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
    { id:"QRY-AGT009-CAMPUS", name:"Campus and healthcare facilities", queryText:"(campus OR hospital OR university) AND (asset information OR energy management OR IoT OR reality capture)", sourceCategory:"Universities + healthcare systems", geography:"United States", cadence:"Weekdays 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Official procurement and capital-plan pages only", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
    { id:"QRY-AGT009-PRIVATE", name:"Private owner expansion signals", queryText:"(facility expansion OR new campus OR portfolio modernization) AND (digital twin OR BIM OR building data)", sourceCategory:"Official company news, filings, careers, and planning sources", geography:"United States", cadence:"Weekly Monday 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Official public company sources only; privacy-minimized", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
  ];
  ...
  await db.insert(agentMessages).values({ id:"MSG-AGT009-ACTIVATION", sharedGoalId:SCOUT_GOAL_ID, correlationId:SCOUT_ACTIVATION_CORRELATION_ID, senderAgentId:"AGT-001", recipientAgentId:"AGT-009", messageType:"Information", status:"Accepted", subject:"AGT-009 bounded pilot approved", bodyJson:JSON.stringify({ authority:"L1", owner:"AGT-001", customer:"AGT-002", approvalBoundary:"No external contact, account creation, terms acceptance, bid submission, or commitment", monitoringQueryCount:queries.length }), idempotencyKey:"activate:agt-009:v1", acceptedAt:now }).onConflictDoNothing();
