...
async function activateScout() {
  ...
  const objective="Continuously discover and route verified New York and New Jersey state and local technology-infrastructure work matching Symbiont's historical strengths and approved new prerogatives, without unauthorized external action.";
  const successMeasures=["Canonical source opened before reporting","Network cabling, switching, voice, access control, surveillance, IT consulting, 3D scanning, and building-data lanes monitored","Historical matches cite governed knowledge and new prerogatives remain labeled","No duplicate handoffs","AGT-002 explicitly accepts pursuit handoffs","Zero unauthorized external actions"];
  await db.insert(sharedGoals).values({ id:SCOUT_GOAL_ID, objective, accountableAgentId:"AGT-001", priority:"P1", status:"Active", successMeasuresJson:JSON.stringify(successMeasures), updatedAt:now }).onConflictDoUpdate({ target:sharedGoals.id, set:{ objective, accountableAgentId:"AGT-001", priority:"P1", status:"Active", successMeasuresJson:JSON.stringify(successMeasures), updatedAt:now } });
  const assignments = [
  ...
  const queries = [
    { id:"QRY-AGT009-NY-STATE", name:"New York state technology infrastructure", queryText:"(network cabling OR fiber OR switching OR telephone OR unified communications OR access control OR security camera OR IT consulting OR 3D scanning OR reality capture OR BIM) AND (RFP OR RFQ OR bid OR capital project)", sourceCategory:"NYS Contract Reporter, OGS, authorities, SUNY/CUNY and official issuer sources", geography:"New York", cadence:"Weekdays 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Official public pages and APIs only; no account creation or terms acceptance", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
    { id:"QRY-AGT009-NY-LOCAL", name:"New York local technology projects", queryText:"(structured cabling OR LAN OR switching OR voice OR access control OR CCTV OR IT services OR laser scanning OR BIM OR asset information) AND (renovation OR modernization OR infrastructure OR procurement)", sourceCategory:"NYC, counties, municipalities, schools, transit, airports and utilities", geography:"New York", cadence:"Weekdays 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Canonical issuer sources only; respect restricted-period contact rules", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
    { id:"QRY-AGT009-NJ-STATE", name:"New Jersey state technology infrastructure", queryText:"(network cabling OR fiber OR switching OR telephone OR VoIP OR access control OR surveillance OR ALPR OR IT consulting OR 3D scanning OR reality capture OR BIM) AND (RFP OR RFQ OR bid)", sourceCategory:"NJ Treasury DPP, NJSTART public notices, NJDOT, NJ Transit and official issuer sources", geography:"New Jersey", cadence:"Weekdays 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Use public notices; stop at authentication, CAPTCHA or click-through terms", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
    { id:"QRY-AGT009-NJ-LOCAL", name:"New Jersey local technology projects", queryText:"(structured cabling OR network equipment OR telephone OR access control OR security camera OR IT services OR laser scanning OR BIM OR asset data) AND (facility OR campus OR school OR utility OR transit)", sourceCategory:"Counties, municipalities, schools, universities, authorities and utilities", geography:"New Jersey", cadence:"Weekdays 08:00 ET", status:"Active", nextCheckAt:tomorrow, robotsPolicy:"Official procurement and capital-plan sources only; privacy-minimized", requiresAuthentication:false, ownerAgentId:"AGT-009", updatedAt:now },
  ];
